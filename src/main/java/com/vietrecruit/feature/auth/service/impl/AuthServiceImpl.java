...
public class AuthServiceImpl implements AuthService {
        ...
        public void verifyEmail(String token) {
                ...
                User user = userRepository
                                .findById(userId)
                                .orElseThrow(() -> new ApiException(ApiErrorCode.AUTH_VERIFY_TOKEN_INVALID));
                                ...
                                .ifPresent(
                                                user -> {
                                                        ...
                                                        String resetLink = String.format(
                                                                        "%s/reset-password?token=placeholder",
                                                                        frontendBaseUrl);
        ...
        private LoginResponse buildLoginResponse(User user) {
                ...
                Set<String> roleCodes = userWithRoles.getRoles().stream().map(Role::getCode)
                                .collect(Collectors.toSet());

                Set<String> permissionCodes = userWithRoles.getRoles().stream()
                                .flatMap(role -> role.getPermissions().stream())
                                .map(Permission::getCode)
                                .collect(Collectors.toSet());
